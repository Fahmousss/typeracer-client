...
export function LeaderboardContent() {
  const { data, error } = useSWR("/api/user-record", fetcher);

  // Error handling and loading state
  if (error) return <Skeleton className="w-56 h-36" />;
  if (!data) return <Skeleton className="w-56 h-36" />;
          ...
          <div className="w-full" key={index}>
            <div className="flex flex-row gap-7 p-2 justify-between">
              <div className="flex flex-row gap-6 pl-3 truncate">
                ...
                <p className="text-xs">{record.user?.name ?? "unknown user"}</p>
              </div>
